...
from common_ml.utils import dict_to_str

# This function is used to format the tags generated by the various services into the format expected by the search and 
# video editor. It operates on a content write token directly which should be published after the tags are uploaded. 
# 
# Args:
...
def format_fabric_tags(client: ElvClient, write_token: str, streams: List[str], interval: int) -> None:
    ...
    for stream in streams:
        ...
        for feature in os.listdir(os.path.join(save_path, write_token, stream)):
            ...
            stream_info = client.content_object_metadata(object_id=write_token, metadata_subtree=f'offerings/default/media_struct/streams/{stream}')
...
def merge_video_tag_files(tags: List[str], tag_duration: float) -> List[VideoTag]:
