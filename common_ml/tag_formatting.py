...
def format_video_tags(client: ElvClient, write_token: str, streams: List[str], interval: int) -> None:
    ...
    for stream in streams:
        ...
        for feature in os.listdir(os.path.join(save_path, write_token, stream)):
            ...
            stream_info = _fetch_stream_info(write_token, stream, client)
            assert len(stream_info.get("sources", [])) > 0, f"No sources found for stream {stream}"
            part_duration = stream_info["sources"][0]["duration"]["float"]
            tags_path = os.path.join(save_path, write_token, stream, feature)
            if stream_info["codec_type"] == "video":
                frames_per_sec = int(stream_info["rate"].split("/")[0]) / int(stream_info["rate"].split("/")[1])
                assert fps is None or fps == frames_per_sec, "FPS mismatch, all tagged streams should have the same FPS"
                fps = frames_per_sec
                frames_per_part = part_duration * frames_per_sec
...
def _fetch_stream_info(write_token: str, stream_name: str, client: ElvClient) -> dict:
    try:
        transcodes = client.content_object_metadata(write_token=write_token, metadata_subtree='transcodes', resolve_links=False)
    except HTTPError:
        logger.warning(f"Transcodes not found. Falling back to legacy method.")
        return _fetch_stream_info_legacy(write_token, stream_name, client)
    
    try:
        streams = client.content_object_metadata(write_token=write_token, metadata_subtree='offerings/default/playout/streams')
    except HTTPError as e:
        raise HTTPError(f"Failed to retrieve streams") from e   
    ...
    for repr in representations.values():
        ...
        if transcode_id is None:
            ...
            continue
        if tid != transcode_id:
            logger.error(f"Multiple transcode_ids found for stream {stream_name}! Continuing with the first one found.")

    if transcode_id is None:
        raise ValueError(f"Transcode_id not found for stream {stream_name}")

    return transcodes[transcode_id]["stream"]

def _fetch_stream_info_legacy(write_token: str, stream_name: str, client: ElvClient) -> dict:
    try:
        streams = client.content_object_metadata(write_token=write_token, metadata_subtree='offerings/default/media_struct/streams')
    except HTTPError as e:
        raise HTTPError(f"Failed to retrieve streams for write_token {write_token}") from e
    if stream_name not in streams:
        raise ValueError(f"Stream {stream_name} not found")
